...
if Config.UseBlips then
  CreateThread(function()
    for k, v in pairs(WheatFields) do
      ...
      WheatFieldLocation:onPlayerInOut(function(isPointInside)
        if isPointInside then
          ...
          for k, v in pairs(WheatField1) do
            exports['qb-target']:RemoveZone(v.Name)
            inZone = false
          end
        end
...
RegisterNetEvent('qb-simplefarming:wheatfield', function()
  QBCore.Functions.Progressbar("picking_wheat", Config.Alerts['wheat_picking'], 3000, false, true, {
...
RegisterNetEvent('qb-simplefarming:makegrain', function()
  QBCore.Functions.TriggerCallback('qb-simplefarming:wheatcheck', function(grain)
    if grain then
      TriggerEvent('animations:client:EmoteCommandStart', {"Clipboard"})
      QBCore.Functions.Progressbar('food_processing', Config.Alerts['wheat_progressbar'], Config.ProcessingTime['pumpkin_smashingtime'] , false, true, {
      disableMovement = true,
      disableCarMovement = true,
      disableMouse = false,
      disableCombat = true,
    ...
    elseif not grain then
      QBCore.Functions.Notify(Config.Alerts['error_grain'], "error", 3000)
    end
  end)
end)


RegisterNetEvent('WheatField', function()
  for k, v in pairs(WheatField1) do
    exports['qb-target']:AddBoxZone(v.Name, vector3(v.Coords.x, v.Coords.y, v.Coords.z), v.length, v.width, {
      name=v.Name,
      heading= v.heading,
      ...
      options = {
        {
          type = "client",
          event = "qb-simplefarming:wheatfield",
          icon = "fa fa-sign-language",
          label = "Gather Wheat",
        },
      },
...
RegisterNetEvent('qb-simplefarming:menuprocessor', function(data)
  local Processor = {
    ...
    },
    {
        header = 'Make Grain',
        params = {
            event = 'qb-simplefarming:makegrain',
        }
    },
    --[[ {
      header = 'Make Canned Corn',
      params = {
