...
QBCore.Functions.CreateCallback('qb-simplefarming:wheatcheck', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player ~= nil then
        if Player.Functions.GetItemByName("wheat") ~= nil then
            cb(true)
        else
            cb(false)
        end
...
RegisterServerEvent('qb-simplefarming:wheatpicking', function()
    ...
    Player.Functions.AddItem('wheat', wheatfarming)
    TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items['wheat'], "add")
    TriggerClientEvent('QBCore:Notify', source, "Grabbed " ..wheatfarming.. " Wheat")
end)

RegisterServerEvent('qb-simplefarming:wheatprocessing', function()
    local source = source
    local Player = QBCore.Functions.GetPlayer(tonumber(source))
    local wheat = Player.Functions.GetItemByName('wheat')
    if not wheat then
        TriggerClientEvent('QBCore:Notify', source, Config.Alerts['error_wheat'])
        return false
    end
    ...
    if not Player.Functions.RemoveItem('wheat', amount) then
        TriggerClientEvent('QBCore:Notify', source, Config.Alerts['itemamount'])
        return false
    end
    ...
    local amount = Config.WheatProcessed
    Wait(750)
    Player.Functions.AddItem('grain', amount)
    TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items['grain'], "add")
